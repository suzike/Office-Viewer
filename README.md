...
<p align="center">
  <strong>面向 Windows 的独立文档查看、编辑与 AI 交互工作台</strong><br>
  无需 VS Code，即可在统一桌面界面中处理 Office、Markdown、PDF、图片、压缩包和开发者文档。
</p>

<p align="center">
  <a href="https://github.com/suzike/Office-Viewer/releases/latest"><img src="https://img.shields.io/github/v/release/suzike/Office-Viewer?display_name=tag&sort=semver" alt="Release"></a>
  <img src="https://img.shields.io/badge/platform-Windows%2010%2F11-0078D4?logo=windows" alt="Windows 10/11">
  <img src="https://img.shields.io/badge/Electron-43.1.1-47848F?logo=electron" alt="Electron 43.1.1">
  <a href="LICENSE"><img src="https://img.shields.io/github/license/suzike/Office-Viewer" alt="MIT License"></a>
...
| 模块 | 能力 |
| --- | --- |
| 多文档工作区 | 原生 Windows 窗口、拖放打开、多标签、最近文件、文件元数据、深浅色模式、单实例文件转交 |
...
| Markdown | 所见即所得、目录、表格、公式、Mermaid、WikiLink、主题、查找替换，以及 PDF/DOCX/HTML 导出 |
...
| 开发者工具 | Git History、HTTP/REST 请求、HTML 预览、Java Class 反编译、JSON/YAML/XML/TOML/代码文本查看 |
| AI 文档助手 | 当前文档问答、总结、改写、翻译、风险检查、选区交互、流式响应和多模型切换 |
...
- 本地 Claude Code CLI；
- Ollama；
- OpenAI 兼容接口，包括 DeepSeek、Kimi 及自定义服务；
...
Codex CLI 目前标记为高信任实验模式，因为 CLI 自身无法完全关闭本地读取工具。使用任何远程模型前，请确认文档内容满足组织的数据安全要求。
...
- `Office Viewer Setup 0.5.1.exe`：标准安装版；
- `Office Viewer 0.5.1.exe`：无需安装的便携版。
...
```mermaid
flowchart LR
    Shell["Windows / Electron Main"] --> IPC["受限 IPC 与文件会话"]
    IPC --> UI["React 多文档桌面界面"]
    UI --> Office["Word / Excel / PowerPoint 渲染器"]
    UI --> Docs["Markdown / PDF / 图片 / 压缩包"]
    Office --> Workers["解析 Worker + 有界 LRU 缓存"]
    Docs --> Workers
    UI --> Assistant["文档 AI 助手"]
    Assistant --> Context["主进程上下文提取与缓存"]
    Context --> Providers["Codex / Claude / Ollama / 第三方 LLM"]
```
