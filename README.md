...
助手还支持划词浮动操作（解释 / 翻译 / 引用到助手）、斜杠命令与自定义快捷动作、提示词库、历史消息编辑重发、对话导出 Markdown、Ollama 模型自动发现和全局唤醒快捷键（Ctrl+Shift+Space）。向远程模型发送前会检测私钥、Token、AWS 密钥、邮箱和身份证号等敏感数据，并要求确认后才会发出；temperature 与 maxTokens 等模型参数可按提供器分别调节。
