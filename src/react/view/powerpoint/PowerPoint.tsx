...
import { officeSessionCacheKey, type OfficeCachePayload } from "../../util/officeSessionCache";
...
const presentationCache = new BoundedLruCache<string, CachedPresentation>({
    maxEntries: 3,
    maxWeight: 384 * 1024 * 1024,
    weigh: entry => entry.sourceBytes + entry.thumbnails.reduce((size, value) => size + value.length * 2, 0),
    dispose: entry => entry.viewer.destroy(),
});
...
async function generateThumbnails(viewer: PPTXViewer, count: number): Promise<string[]> {
    ...
    for (let i = 0; i < count; i++) {
        ...
        // pptxviewjs currently requires an HTMLCanvasElement and therefore
        // cannot render in a Worker. Yield between slides to keep input and
        // tab switching responsive while the bounded viewer cache is warmed.
        await new Promise<void>(resolve => requestAnimationFrame(() => resolve()));
    }
    return urls;
}
...
export default function PowerPoint() {
    ...
    const loadPresentation = useCallback(async (payload: OfficeCachePayload & { path?: string; buffer?: number[]; error?: string }) => {
        ...
        try {
            ...
            if (cached) {
                performance.mark('office-powerpoint-cache-hit');
                cached.viewer.setCanvas(canvas);
                viewerRef.current = cached.viewer;
                const count = cached.viewer.getSlideCount();
                const activeIndex = cached.viewer.getCurrentSlideIndex();
                setSlideCount(count);
                setCurrentIndex(activeIndex);
                setThumbnails(cached.thumbnails);
                await cached.viewer.render(canvas);
                return;
            }

            performance.mark('office-powerpoint-cache-miss');
            ...
            const viewer = new PPTXViewer({
                ...
                // pptxviewjs schedules an uncancellable async re-render by default.
                // Disable it because a document-tab switch may destroy the viewer first.
            ...
            const cacheEntry: CachedPresentation = {
                viewer,
                thumbnails: [],
                sourceBytes: buffer.byteLength,
            };
            presentationCache.set(cacheKey, cacheEntry);

            setThumbsLoading(true);
            const urls = await generateThumbnails(viewer, count);
            cacheEntry.thumbnails = urls;
            presentationCache.set(cacheKey, cacheEntry);
            setThumbnails(urls);
            applyCanvasSize(canvas, wrap.clientWidth, wrap.clientHeight);
