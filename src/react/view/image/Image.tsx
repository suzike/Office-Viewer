...
import { hasImageBuffer, type ImageSource, needsConversion, resolveImageSrc, revokeObjectUrl } from './convertImage';
import './Image.less';
...
export default function Image() {
    ...
    useEffect(() => {
        handler.on('images', info => {
            ...
            const hasResolvable = info.images.some((image: ImageSource) =>
                needsConversion(image) || hasImageBuffer(image)
            );
    ...
    return (
        <div className="image-viewer">
            {loading && <ViewerState kind="loading" />}
