...
function resolveRelativeEpubHref(sectionHref: string, href: string): string | null {
    if (!href || /^(?:[a-z][a-z\d+.-]*:|\/\/)/i.test(href)) {
        return null;
    }
    try {
        const sectionUrl = new URL(sectionHref.replace(/^\/+/, ''), 'https://epub.invalid/');
        const target = new URL(href, sectionUrl);
...
export default function Epub() {
    ...
    const bindRendition = useCallback((rendition: Rendition) => {
        bindWheel(rendition);
        rendition.hooks.content.register((contents: { document?: Document; sectionIndex?: number }) => {
            const book = bookRef.current;
            const doc = contents.document;
            const section = book?.spine.get(contents.sectionIndex ?? 0);
            if (!book || !doc || !section) {
                return;
            }
            for (const link of Array.from(doc.querySelectorAll<HTMLAnchorElement>('a[href]'))) {
                const target = resolveRelativeEpubHref(section.href, link.getAttribute('href') ?? '');
                if (!target) {
                    continue;
                }
                // Without epub.js' file:// <base>, its stock link handler uses
                // the desktop shell URL as the base. Override only archive-local
                // links and leave external/mail links on the original handler.
                link.onclick = (event) => {
                    event.preventDefault();
                    event.stopPropagation();
                    rendition.display(target).catch(() => setError('Failed to open EPUB link'));
                    return false;
                };
            }
        });
    ...
    const loadDocument = useCallback(async (payload: { path?: string; buffer?: number[]; error?: string }) => {
        ...
        try {
            ...
            setMeta({
                ...
                description: sanitizeEpubMetadataHtml(metadata.description),
