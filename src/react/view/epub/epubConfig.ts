...
const EPUB_METADATA_TAGS = new Set([
    'B',
    'BLOCKQUOTE',
    'BR',
    'EM',
...
/**
 * Preserve harmless description formatting while stripping executable metadata.
 * No attributes are retained, so event handlers, URLs and CSS cannot cross the
 * EPUB metadata boundary into the desktop renderer.
 */
export function sanitizeEpubMetadataHtml(value: unknown): string {
    ...
    const copyNode = (node: Node, parent: Node) => {
        ...
        const nextParent = EPUB_METADATA_TAGS.has(element.tagName)
            ? parent.appendChild(output.createElement(element.tagName.toLowerCase()))
            : parent;
...
export async function loadEpubBook(buffer: ArrayBuffer, timeoutMs = 10_000) {
    ...
    try {
        await Promise.race([
            ...
            new Promise<never>((_, reject) => {
                timeout = setTimeout(() => reject(new Error('Timed out while reading EPUB package')), timeoutMs);
            }),
        ]);
        // epub.js normally injects a <base href="file://…"> into every spine
        // document. The desktop CSP intentionally blocks file base URIs, and
        // archived EPUB resources are already rewritten to blob URLs by the
        // library's serialize hook. Keep canonical/meta/link hooks intact while
        // removing only that incompatible base-tag hook.
        const contentHooks = book.spine.hooks.content.list() as Function[];
        const replaceBaseHook = contentHooks.find((hook) => {
            if (hook.name === 'replaceBase') {
                return true;
            }
            const source = Function.prototype.toString.call(hook);
            return /createElement\((['"`])base\1\)/.test(source) && source.includes('setAttribute');
        });
        // Production minification removes the function's source name, so the
        // implementation signature is used as a narrow compatibility fallback.
        if (typeof replaceBaseHook === 'function') {
            book.spine.hooks.content.deregister(replaceBaseHook);
        }
        return book;
    } catch (error) {
        book.destroy();
