...
export interface SvgViewerProps {
    /** Route exports through the desktop host instead of a browser download. */
    hostExports?: boolean;
}

function SvgViewerInner({ hostExports = false }: SvgViewerProps) {
    ...
    const previewUrl = useMemo(() => {
        ...
        const safeContent = sanitizeSvgForRendering(trimmed);
        if (!safeContent) {
            return '';
        }
        const blob = new Blob([ensureSvgNamespace(safeContent)], { type: 'image/svg+xml;charset=utf-8' });
    ...
    const onExportPng = async () => {
        ...
        try {
            if (hostExports) {
                const png = await renderSvgAsPng(content);
                handler.emit('saveAs', {
                    content: new Uint8Array(await png.arrayBuffer()),
                    ext: 'png',
                    mode: 'export',
                });
            } else {
...
export default function SvgViewer({ hostExports = false }: SvgViewerProps) {
    return (
        <App>
            <SvgViewerInner hostExports={hostExports} />
        </App>
    );
}
