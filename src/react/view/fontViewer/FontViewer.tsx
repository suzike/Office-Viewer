...
export default function FontViewer() {
    ...
    useEffect(() => {
        handler.on("open", async (content) => {
            ...
            try {
                ...
                setError(reason instanceof Error ? reason.message : 'Failed to load font file')
            } finally {
                setLoading(false)
            }
        }).emit("init")
    ...
    return (
        <Flex className="font-viewer">
            <div style={{ background: 'var(--vscode-editor-background, #fff)', overflow: 'auto', height, flexGrow: 1 }}>
                {loading && <Spin fullscreen tip="Loading font…" />}
                {!loading && error && (
                    <div style={{ padding: 24 }}>
                        <Alert type="error" showIcon message="Unable to open font" description={error} />
                    </div>
                )}
