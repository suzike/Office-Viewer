...
export async function exportSaveAs(
    ...
    if (ext === 'xls' || ext === 'ods') {
        // xlsx（SheetJS）约 1MB，仅在导出 .xls/.ods 时按需加载
        const { writeSheetJsWorkbook } = await import('./excel_sheetjs');
        const buf = writeSheetJsWorkbook(sheets, ext);
        handler.emit('saveAs', { content: [...buf], ext });
        return;
    }
    if (ext === 'csv' || ext === 'tsv') {
        const fs = ext === 'tsv' ? '\t' : csvDelimiter;
        const { sheetDataToCsv } = await import('./excel_sheetjs');
        const csvContent = sheetDataToCsv(sheets[0], fs);
