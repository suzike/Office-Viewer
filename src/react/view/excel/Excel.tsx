...
import { useViewerColorMode } from "../../util/viewerColorMode.ts";
import { loadOfficeBuffer } from "../../util/loadOfficeContent.ts";
import { officeSessionCacheKey } from "../../util/officeSessionCache.ts";
...
import { loadParsedExcelWorkbook } from "./excel_parse_client.ts";
...
const EXCEL_COLOR_MODE_OPTIONS = {
    storageKey: 'office-excel-color-mode',
    stateKey: 'excelColorMode',
    legacyDarkModeKey: 'office-dark-mode',
} as const;
...
function ExcelViewer() {
    ...
    const { adaptiveColorMode, themedDark, toggleColorMode } = useViewerColorMode(EXCEL_COLOR_MODE_OPTIONS)
    ...
    useEffect(() => {
        ...
        const initSpreadsheet = async (buffer: ArrayBuffer, payload: any, cacheKey: string) => {
            const fileReadOnly = payload.readOnly === true;
            if (payload.ext?.match(/csv/i)) {
                csvEncodingRef.current = detectCsvEncoding(buffer);
            }
            const { sheets, maxLength, maxCols, csvDelimiter } = window.officeDesktop
                ? await loadParsedExcelWorkbook(buffer, payload.ext, cacheKey)
                : await loadSheets(buffer, payload.ext);
            if (disposed) return;
            if (csvDelimiter) {
        ...
        handler.on("open", (payload) => {
            ...
            if (loadedCacheKeyRef.current === cacheKey && spreadSheetRef.current) {
            ...
            loadOfficeBuffer(payload).then(async (buffer) => {
                try {
                    await initSpreadsheet(buffer, payload, cacheKey);
                } catch (e) {
