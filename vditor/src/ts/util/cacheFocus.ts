...
export const restoreCacheFocus = (vditor: IVditor, options?: RestoreFocusOptions) => {
    ...
    const apply = () => {
        const editor = vditor[vditor.currentMode].element;
        const activeElement = document.activeElement;
        const userFocusedOutsideEditor = activeElement instanceof HTMLElement
            && activeElement !== document.body
            && activeElement !== document.documentElement
            && !editor.contains(activeElement);
        // The two-frame restore runs after initial rendering. Do not let it steal
        // focus from a toolbar control or close a panel the user opened meanwhile.
        if (hasOpenEditorOverlay(vditor) || userFocusedOutsideEditor) {
            if (onLoad) {
