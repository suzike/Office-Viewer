...
export const setEditorTheme = (
    ...
    const alreadyApplied = previous === resolved
        && vditor.element.getAttribute("data-editor-theme") === resolved;
    if (alreadyApplied) {
    ...
    if (resolved !== "Auto") {
        ...
        const historyPatch: Record<string, string> = {
            [LAST_NON_AUTO_EDITOR_THEME_KEY]: resolved,
        };
        if (EDITOR_DARK_THEMES.has(resolved)) {
            vditor.options.lastDarkEditorTheme = resolved;
            historyPatch[LAST_DARK_EDITOR_THEME_KEY] = resolved;
        } else {
            vditor.options.lastLightEditorTheme = resolved;
            historyPatch[LAST_LIGHT_EDITOR_THEME_KEY] = resolved;
        }
        setGlobalLocalStorageSettings(historyPatch);
    }
